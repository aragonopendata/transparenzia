class AgreementController < ApplicationController
  def index
    ...
    @agreements = Agreement.where(:agreement_date => year_ini..year_end).where("lower(title) like lower(?)", title).where("lower(signatories) like lower(?)", signatories).paginate(:page => params[:page], :per_page => 5)
  end
